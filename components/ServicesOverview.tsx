        ...
        >
          {SERVICES.pillars.map((pillar, i) => (
            <motion.div key={pillar.id} variants={card}>
              <Link
                href={`#${pillar.id}`}
                className="group flex flex-col h-full bg-white dark:bg-navy dark:border-white/10 rounded-2xl p-5 border border-navy/8 hover:border-pink/40 hover:shadow-md transition-all duration-200"
              >
                ...
                <h3 className="font-bold text-navy dark:text-white text-sm leading-snug mb-3">
                  {pillar.title}
                </h3>
                <div className="h-px bg-navy/8 dark:bg-white/10 mb-3" />
                <ul className="flex-1 space-y-1.5 mb-4">
                  {pillar.services.map((s) => (
                    <li key={s.name} className="text-xs text-navy/55 dark:text-white/70 leading-snug">
                      {s.name}
                    </li>
                  ))}
                </ul>
